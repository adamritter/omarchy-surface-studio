   ...
   C.ScrollView {
    ...
    ColumnLayout {
     ...
     ColumnLayout {
      ...
      StudioToggle { visible:root.effectPage===0; text:"Ray-traced glass · experimental"; checked:root.studio && root.profileValue("rayGlass"); onClicked:{ root.profileChange("rayGlass",checked); if(checked) root.profileChange("effectsEnabled",true) } }
      Text { visible:root.effectPage===0 && root.profileValue("rayGlass"); Layout.fillWidth:true; wrapMode:Text.WordWrap; text:"Traces light through a virtual glass slab. Uses a virtual environment, not the live desktop. Roughness and light controls also affect the glass."; color:Color.popups.text; opacity:0.65; font.pixelSize:11 }
      StudioToggle { visible:root.effectPage===3 && !root.targetPanel; text:"Bar hover highlights"; checked:root.studio && root.studio.current.barHoverEnabled; onClicked:root.studio.change("barHoverEnabled",checked) }
      Repeater {
       model:[{key:"barHoverStrength",label:"Hover strength",min:0,max:0.25,step:0.01},{key:"barHoverRadius",label:"Hover rounding",min:0,max:16,step:1},{key:"barModuleGap",label:"Widget spacing",min:0,max:8,step:0.5}]
       ColumnLayout {
        required property var modelData
        visible:root.effectPage===3 && !root.targetPanel
        Layout.fillWidth:true
        Text { text:modelData.label+" · "+Number(root.studio?root.studio.current[modelData.key]:0).toFixed(2); color:Color.popups.text; font.pixelSize:13 }
        StudioSlider { Layout.fillWidth:true; from:modelData.min; to:modelData.max; stepSize:modelData.step; value:root.studio?root.studio.current[modelData.key]:0; onMoved:root.studio.change(modelData.key,value) }
       }
      }
