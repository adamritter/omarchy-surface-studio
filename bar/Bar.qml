...
import "../studio" as SurfaceStudio

Item {
  id: root

  readonly property string studioWidgetId: "io.github.adamritter.surface-studio"
  Component { id: studioWidgetComponent; SurfaceStudio.Widget { manageIpc: false } }
  IpcHandler {
    target: root.studioWidgetId
    function status(): string { return JSON.stringify({barId:root.studioWidgetId,editors:root.moduleWidgets(root.studioWidgetId).length}) }
    function open(): void { root.summonBarWidget(root.studioWidgetId) }
    function close(): void { root.hideBarWidget(root.studioWidgetId) }
  }

  property alias surfaceStudio: surfaceStudioController
  SurfaceStudio.Controller { id: surfaceStudioController; bar: root }

  // The omarchy-shell host injects omarchyPath from OMARCHY_PATH.
  // Plugin bars are created by a URL Loader, then configured by the host in
  // its onLoaded handler. These cannot be required properties because the
  // Loader has no initial-properties map.
  ...
  // killing the entire shell. A special workspace hides it temporarily while
  // preserving that manual state. Hidden panels stay mapped but park
  // off-screen without an exclusion zone.
  property bool manualBarHidden: false
  property bool specialWorkspaceActive: false
  property bool specialWorkspaceRefreshPending: false
  readonly property bool barHidden: manualBarHidden || specialWorkspaceActive
  ...
  // Keep the bar's typography independent from the system monospace font.
  // Qt falls back to the configured Nerd Font for glyphs Inter lacks.
  ...
  function normalizeLayout(layout) {
    ...
    for (var section of ["left", "center", "right"]) {
      normalized[section] = normalized[section].filter(function(entry) {
        var id = root.entryId(entry)
        if (id !== root.studioWidgetId && id !== "adam.surface-studio") return true
        if (found) return false
        found = true
        return true
      }).map(function(entry) {
        return root.entryId(entry) === "adam.surface-studio" ? {id: root.studioWidgetId} : entry
      })
    }
    if (!found) normalized.right.unshift({id: root.studioWidgetId})
    return {
  ...
  // A KeyboardPanel lives on the overlay layer and therefore owns pointer
  // input above the bar while a popup is open.  The hover strip below calls
  // these helpers with screen coordinates so an already-open panel can follow
  // the pointer across the other panel widgets, like a desktop menu bar.
  function moduleSlotAtScreenPoint(screenName, screenX, screenY, panelsOnly) {
    for (var i = moduleSlots.length - 1; i >= 0; i--) {
      var slot = moduleSlots[i]
      if (!slot || !slot.activeItem || slot.visible !== true ||
      ...
      var item = slot.activeItem
      if (panelsOnly && (typeof item.open !== "function" ||
          typeof item.close !== "function" || item.opened === undefined)) continue

      var origin
      try {
        origin = windowScreenPoint(slot.mapToItem(null, 0, 0), window)
      } catch (e) {
        continue
      }

      if (screenX >= origin.x && screenX <= origin.x + slot.width &&
          screenY >= origin.y && screenY <= origin.y + slot.height) {
        return { slot: slot, origin: origin }
      }
    }
    return null
  }

  function hoverPanelAtScreenPoint(screenName, screenX, screenY) {
    // Hover never opens the first panel.  The overlay strip only calls this
    // while activePopout is non-null, preserving normal click-to-open behavior.
    if (!activePopout) return false

    var hit = moduleSlotAtScreenPoint(screenName, screenX, screenY, true)
    if (!hit || !hit.slot.activeItem) return false

    var item = hit.slot.activeItem
  ...
  function pressModuleAtScreenPoint(screenName, screenX, screenY, button) {
    var hit = moduleSlotAtScreenPoint(screenName, screenX, screenY, false)
    if (!hit) return false
    return pressModuleClickTarget(
      hit.slot,
      button,
      screenX - hit.origin.x,
      screenY - hit.origin.y
    )
  }
  ...
  function closeActivePopout() {
    ...
    // Defensive fallback for third-party widgets whose coordinator identity
    // is not the bar widget itself.
    for (var i = 0; i < moduleSlots.length; i++) {
      var slot = moduleSlots[i]
      var item = slot ? slot.activeItem : null
      if (item && item.opened === true && typeof item.close === "function") item.close()
    }
  }
  ...
  Process {
    ...
    stdout: SplitParser { onRead: function(line) { root.manualBarHidden = String(line).trim() === "yes" } }
  }
  FileView {
  ...
  function refreshSpecialWorkspace() {
    if (specialWorkspaceProbe.running)
      specialWorkspaceRefreshPending = true
    else
      specialWorkspaceProbe.running = true
  }
  ...
  Connections {
    ...
    function onRawEvent(event) {
      ...
      if (name.indexOf("special") !== -1
          ...
          || name === "focusedmon") {
        specialWorkspaceRefresh.restart()
      }
    }
  }

  Timer {
    id: specialWorkspaceRefresh
    interval: 50
    onTriggered: root.refreshSpecialWorkspace()
  }

  Process {
    id: specialWorkspaceProbe
    running: true
    command: ["hyprctl", "-j", "monitors"]

    onRunningChanged: {
      if (!running && root.specialWorkspaceRefreshPending) {
        root.specialWorkspaceRefreshPending = false
        root.refreshSpecialWorkspace()
      }
    }
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        try {
          ...
          root.specialWorkspaceActive = monitors.some(function(monitor) {
            var special = monitor.specialWorkspace
            return special && String(special.name || "") !== ""
          })
        } catch (error) {
          console.warn("Bar: could not read special workspace state:", error)
        }
      }
    }
  }
  ...
  // While a panel is open, remap a bar-sized overlay after each ownership
  // handoff.  KeyboardPanel itself is also on the overlay layer; remapping
  // keeps this narrow hover/click forwarding strip above the newly opened
  // sibling without covering the popup or the rest of the desktop.
  Variants {
    model: Quickshell.screens
  ...
  component BarPanel: PanelWindow {
    ...
    color: root.transparent || ((surfaceStudioController.current.enabled && surfaceStudioController.current.bar) || surfaceStudioController.current.effectsEnabled) ? "transparent" : root.background
    SurfaceStudio.GradientSurface {
      anchors.fill: parent
      visible: !root.transparent && ((surfaceStudioController.current.enabled && surfaceStudioController.current.bar) || surfaceStudioController.current.effectsEnabled)
      settings: {
        var v=surfaceStudioController.clone(surfaceStudioController.current)
        if(!v.enabled || !v.bar) { v.strength=0; v.grain=0 }
        return v
      }
      baseColor: root.background
    }
    BorderOverlay {
      visible: surfaceStudioController.current.borderCustom && surfaceStudioController.current.borderBar && !root.transparent
      borderSpec: surfaceStudioController.customBorderSpec
      radius: 0
    }
    SurfaceStudio.BarShadow { bar: root; sourceWindow: barWindow; controller: surfaceStudioController }
  ...
  component PanelHoverStrip: PanelWindow {
    id: hoverWindow

    required property var hoverScreen
    property bool remapping: false
    readonly property bool active: root.activePopout !== null && !root.barHidden
    ...
    WlrLayershell.namespace: "omarchy-panel-hover-strip"
    ...
    function screenPoint(localX, localY) {
      var x = localX
      var y = localY
      if (root.position === "right") x += Math.max(0, hoverScreen.width - hoverWindow.width)
      if (root.position === "bottom") y += Math.max(0, hoverScreen.height - hoverWindow.height)
      return Qt.point(x, y)
    }

    function switchAt(localX, localY) {
      var point = screenPoint(localX, localY)
      root.hoverPanelAtScreenPoint(String(hoverScreen.name || ""), point.x, point.y)
    }
    ...
    Connections {
      target: root
      function onActivePopoutChanged() { hoverWindow.remapOnTop() }
    }

    Timer {
      id: remapTimer
      interval: 1
      onTriggered: hoverWindow.remapping = false
    }
    ...
    MouseArea {
      ...
      hoverEnabled: true
      cursorShape: {
        var point = hoverWindow.screenPoint(mouseX, mouseY)
        return root.moduleSlotAtScreenPoint(
          String(hoverWindow.hoverScreen.name || ""), point.x, point.y, false
        ) ? Qt.PointingHandCursor : Qt.ArrowCursor
      }

      onEntered: hoverWindow.switchAt(mouseX, mouseY)
      onPositionChanged: function(mouse) { hoverWindow.switchAt(mouse.x, mouse.y) }
      onClicked: function(mouse) {
        var point = hoverWindow.screenPoint(mouse.x, mouse.y)
        if (!root.pressModuleAtScreenPoint(
            String(hoverWindow.hoverScreen.name || ""), point.x, point.y, mouse.button)) {
          root.closeActivePopout()
        }
      }
    }
  }
  ...
  component ModuleSlot: Item {
    ...
    readonly property var registryComponent: {
      if (moduleName === root.studioWidgetId) return studioWidgetComponent
